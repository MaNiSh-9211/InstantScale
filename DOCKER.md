...
`hotpod-devel` (gcc 14 + CRIU v4.1 source-build capability), built from
...
Compose services: `source` (`hotpod-src`: warm app â†’ checkpoint â†’ pageserver
...
|---|---|---|
| Phases 1â€“4 full matrix | `powershell -ExecutionPolicy Bypass -File hotpod.ps1 all` | `make test-mvp test-p2 test-p3` |
| Phase 1 kernel prototype | `hotpod.ps1 mvp` | `make -C mvp clean all && ./mvp/uffd_selffault` |
| Phase 2 LAZY-vs-EAGER demo | `hotpod.ps1 p2` | `make -C phase2 clean all && bash phase2/demo.sh` |
| Phase 3 lifecycle battery | `hotpod.ps1 p3` | `bash phase3/battery.sh` |
| Two-host migration | `hotpod.ps1 mh` | see compose above or `phase4/multihost.ps1` |
| Stress hammer | `hotpod.ps1 hammer` | `bash phase2/hammer.sh 64 10` |

## Manual build & run (per binary, inside a container)

```powershell
# build everything
docker run --rm --privileged -v "${PWD}:/src" -w /src hotpod-devel `
...
docker run -d --name hotpod-src --privileged --network hotpod-net `
  -v "${PWD}:/src" -w /src hotpod-devel `
  ./phase2/pageserver --port 46100 --image /src/artifacts/app.isim

# run restorer against it (terminal 2)
docker run --rm --privileged --network hotpod-net -v "${PWD}:/src" -w /src `
  hotpod-devel ./phase3/demo_app --resume-lazy-img artifacts/app.isim `
  --host hotpod-src --port 46100
```
...
| fan-out replica logs | `/tmp/hotpodN.*/r*.log` inside the container |
| CI diagnostics | Actions â†’ run â†’ artifact `criu-lazy-logs` (criu.yml) |

## Health probes

| Component | Probe (inside its network/container) |
|---|---|
| pageserver listening | `bash -c 'exec 3<>/dev/tcp/127.0.0.1/46100'` â†’ exit 0 |
| checkpoint produced | `[ -f artifacts/checkpoint.ready ]` and non-empty `artifacts/app.isim` |
| restored instance alive & hydrated | heartbeat file gains `HB seq=` lines; on SIGTERM prints `FINAL digest=0xâ€¦` matching source |
| CRIU present | `criu --version` exits 0 |

## Observability verification (30-second smoke)

```powershell
powershell -File hotpod.ps1 p3     # expect: RESULT mode=lazy activation_ms < 5, FINAL digest=0xâ€¦ present
```
...
Any `digest mismatch` is fatal by design â€” treat as a bug report.
