...
## Decision
Every test path runs inside privileged `hotpod-devel` / `gcc:14` containers
(bind-mounted repo, bash entry points). A single PowerShell runner,
`hotpod.ps1`, exposes phases (`mvp|p2|p3|mh|hammer|all`), auto-starts
...
userfaultfd) â€” acceptable for a dev/test boundary; production guidance keeps
