...
HotPod separates **process activation** from **memory transfer**.
...
| orchestrators | `phase{3,4}/*.sh`, `hotpod.ps1`, compose | sequencing, gates, timing extraction |

## External dependencies

| Dependency | Kind | Required at |
|---|---|---|
| Linux kernel â‰¥ 5.11 (`userfaultfd`) | platform | target runtime |
...
No databases, message brokers, or managed services are used â€” the only
...
|---|---|---|---|---|
| pageserver process dies mid-stream | TCP FIN/RST | `recv == 0` â†’ `ECONNRESET` die in daemon | fail-fast with exact errno | restored instance aborts; orchestrator restarts pair |
...
| duplicate response for already-local page | extra frame | `pend_find` misses â†’ ring park | harmless by immutability | none |
...
|---|---|---|
| handshake + skeleton (META round-trip) | â‰¤ 1 RTT | ~0.3 ms total to `RUNNING` (incl. connect) |
| first-touch resolution (trapâ†’wireâ†’installâ†’wake) | < 250 Âµs p99 | histogram: 2484/<50 Âµs, 755/50â€“100 Âµs, 35/100â€“250 Âµs, 0 above 500 Âµs except 2 |
...
| activation floor cross-host | â‰ˆ RTT | **2.45 ms** docker-bridge hop |
| fan-out spike (10 replicas, one server) | server memcpy + uffd install bound | all RUNNING in 21â€“33 ms wall |
